...
class ModelArgs:
    ...
    multiple_of: int = 128  # MLP hidden layer size will be multiple of
    norm_eps: float = 1e-5
    max_seq_len: int = 256
    use_norms: bool = True
    dropout: float = 0.0
...
class Transformer(MiCoModel):
    ...
    def estimate_loss(self, eval_iters=100):
        ...
        out["TrainLoss"] = losses.mean().item()
        # Test Loss Estimate
        losses = torch.zeros(eval_iters)  # keep on CPU
        test_correct = 0
        for k in range(eval_iters):
            ...
            loss = self.last_loss
            losses[k] = loss.item()
            predict = torch.max(logits.view(-1, logits.size(-1)), dim=1).indices
            correct = Y.view(-1)
            test_correct += (predict == correct).sum().item() / len(predict)
        out["TestLoss"] = losses.mean().item()
        out["TestAcc"] = test_correct / eval_iters
        self.train()
        return out
    
    def test(self, test_loader, train_loader = None):
        self.test_loader = test_loader
        self.train_loader = test_loader if train_loader is None else train_loader
    ...
    def train_loop(self, n_iter, train_loader, test_loader, verbose = False, lr = 0.001, eval_interval = None):
        ...
        for iter_num in iters:
            iters.set_description(f"Step {iter_num}, Loss: {loss.item():.3f}")
