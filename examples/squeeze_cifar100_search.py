...
if __name__ == "__main__":
    ...
    wq_types = [4,5,6,7,8]
    aq_types = [4,5,6,7,8]
    train_loader, test_loader = cifar100(batch_size=batch_size, num_works=4)

    search = MiCoSearch(model, num_epoch, 
                   train_loader, test_loader, seed=0,
                   wq_types=wq_types, aq_types=aq_types,
                   pretrained_model="output/ckpt/squeeze_cifar100.pth")
    ...
    bo_best_res, trace = searcher.search(
        ...
        data_path="output/json/squeeze_cifar100.json",
